...
epsilon = 0.001
outfile = "./Output/BaseCorner"*myInstance*"_"*myRun*".txt"
...
#                             println(f,"Change opt to false")
                            opt = false
                        end
                    end
#                     println(f,"π[v] = ", π[v], ": v = ", v)
                end
            end
        else
            succ = find(pred.==u)
...
#         println("checkedCells = ", checkedCells)
    ...
    if x_count == 10 #===CornerTest===#
        ...
        for k = 1:myLength #k < myLength
            if (k in checkedCells) == false
                ...
                if isOPT == true && z_now[k] - g[k] <= delta1
                    h[k] = g[k]
                    push!(checkedCells, k)
                end
    ...
    myLength = length(p)
    while x_count != 10 && found == false && k < myLength #length(p)# k = 1: cell_num #length(p)
...
#                         push!(Cell_List, newCell)
            ...
            end #if z_now[k] - g[k] <= delta1 
        end #if (k in checkedCells) == false

    end #while x_count != 10 && found == false && k < myLength
...
timesFile = open("/Users/din/Documents/July2020Paper1/Summary/local_BaseCorner.txt", "a")
