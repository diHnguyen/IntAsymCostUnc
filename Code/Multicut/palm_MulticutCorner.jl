...
epsilon = 0.001
outfile = "./Output/MulticutCorner"*myInstance*"_"*myRun*".txt"
...
#                 println("k = ", k)
                if (k in checkedCells) == false
                    ...
                    if g[k]-h[k] > delta2   
                        ...
                        push!(checkedCells, k)
                    end #END OF g[k]-h[k] > delta2
                end #END OF  if (k in checkedCells) == false
            end #END OF while found == false && k < myLength
            @objective(m, Max, sum(p[i]*z[i] for i = 1:length(p)) )
        end #END OF if zCutAdded == false
    end #END OF #===Non-SALoop===#
    time_lapse = toq()    
...
timesFile = open("/Users/din/Documents/July2020Paper1/Summary/local_MulticutCorner.txt", "a")
