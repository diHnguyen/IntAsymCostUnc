...
myInstance = "3" #string(parse(Int64,ARGS[1]))

myRun = Dates.format(now(), "HH:MM:SS")
epsilon = 0.001
outfile = "./Output/MulticutMultipartitionNoRep"*myInstance*"_"*myRun*".txt"
gurobi_env = Gurobi.Env()

to = TimerOutput()
myFile = "./Instances/-5Each_"*myInstance*".jl"
include(myFile)
...
iter = 0
current_cell = 1
# println("No.Iter ; Interdiction ; NumCells ; CurrentMP ; CurrentG ; CurrentH; Num_Hs0; Secs")
...
#             checkedHs = Int64[]
    ...
    current_cell = length(p)
#         print("\n", iter,";", find(x_now.==1), ";", current_cell,";",MP,";")
...
#                 if k == 1
#                     f = open(outfile, "a")
#                         println(f,"y from row: ",find(df[:ARCS_USED][row].==1))
#                         println(f,"cost of y:",sum(c[i]*y[i] + d[i]*y[i]*x_now[i] for i = 1:length(c_L)) )
#                     close(f)
#                 end
...
#                 if k == 1
#                     f = open(outfile, "a")
#                     println(f,"z[k] = ", z_now[k],"; g = ", g[k], "; y = ",find(y.==1))
#                     close(f)
#                 end
...
#                                 println(f,"Constraint ", con_num)
#                                 println(f,"Check constraint: ",find(df[:ARCS_USED][con_num].==1))
#                             close(f)
#                         end
...
#                             if k == 1
#                                 f = open(outfile, "a")
#                                     println(f,"O.C.2: Cell ", k, " is split")
#                                     println(f,"gx_low = ", gx_low," y = ", find(y_low.==1))
#                                     println(f,"gx_high = ", gx_high," y = ", find(y_high.==1))
#                                 close(f)
#                             end
                    y_low_exists = false
                    y_high_exists = false
...
#                                 if k == 1
#                                 f = open(outfile, "a")
#                                 println(f,"2. Constraint ", con_num)
#                                     println(f,"Check constraint: ",find(df[:ARCS_USED][con_num].==1))
#                                 close(f)
#                                 end
                    end
...
#             break
#         end
#         print(sum(p[i]*(g[i]) for i = 1:length(p)),";",sum(p[i]*(h[i]) for i = 1:length(p)),";", length(find(h.==0)),";", total_time)
end #===End While Loop===#
...
timesFile = open("/Users/din/Documents/July2020Paper1/Summary/local_MulticutMultipartitionNoRep.txt", "a")
