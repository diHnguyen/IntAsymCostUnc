...
epsilon = 1e-3
outfile = "./Output/MulticutMultipartition"*myInstance*"_"*myRun*".txt"
...
#                 println(f, "Obj = ", MP_obj)
#                 println(f, "Interdiction ", find(x_now.==1))
#                 println(f, "Cells = ", length(p))
#             close(f)
#             if last_x != getvalue(x) && iter > 1
#                 println("CHANGE INTERDICTION")
#             end        
...
#                 if k == 1
#                     f = open(outfile, "a")
#                     println(f,"z[k] = ", z_now[k],"; g = ", g[k])
#                     close(f)
#                 end
        if z_now[k] - gx >= epsilon
...
# println(f, "Last x = ", last_x)
temp_g = sum(p[i]*g[i] for i = 1:length(p))
temp_h = sum(p[i]*h[i] for i = 1:length(p))
timesFile = open("/Users/din/Documents/July2020Paper1/Summary/local_MulticutMultipartition.txt", "a")
