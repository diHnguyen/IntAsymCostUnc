...
epsilon = 1e-3
outfile = "./Output/MulticutMultipartitionCorner"*myInstance*"_"*myRun*".txt"
...
iter = 0
consecutive_iter = 10
# println("No.Iter ; Interdiction ; NumCells ; CurrentMP ; CurrentG ; CurrentH; Num_Hs0; Secs; CellsRemoved")
while stopping_cond >= delta2 #===Both Timing & While Loop===#
    tic()

    cell_removed_corner = 0
...
#         print("\n", iter,";", find(x_now.==1), ";", length(p),";",MP,";")

    if x_count == consecutive_iter #10 #===CornerTest===#
        iter = iter - 1
        x_count = 0
        ...
        for k = 1:myLength #k < myLength
            if (k in checkedCells) == false
                ...
                g[k] = gx
                isOPT, c_corner = corner_g(y, pred, c_L, c_U, d, x_now, edge, origin, destination)
#                         println("Cell ",k,": ", isOPT)
                if isOPT == true && z_now[k] - gx <= delta1
                    h[k] = gx
                    push!(checkedCells, k)
                    cell_removed_corner = cell_removed_corner + 1
#                             println("HERE")
                end
                stopping_cond = delta2 + 1
            end
        end
    end #===End of CornerTest===#
    if x_count != consecutive_iter #10
        zCutAdded = false  
...
#             break
#         end
#         print(sum(p[i]*(g[i]) for i = 1:length(p)),";",sum(p[i]*(h[i]) for i = 1:length(p)),";", length(find(h.==0)),";", total_time,";", cell_removed_corner)
end #===End While Loop===#
...
timesFile = open("/Users/din/Documents/July2020Paper1/Summary/local_MulticutMultipartitionCorner.txt", "a")
